-- models/intermediate/int_medical_group_plan_awv_summary_monthly.sql
{{
    config(
...
/*
    ...
    medical group and plan level by month.
...
int_aggregate_visits AS (
    ...
    GROUP BY medical_group_id, plan_id, date_month
),
...
int_joined_visits_and_patients AS (
    SELECT 
        ...
        sum(b.visit_count_per_month) OVER (
            PARTITION BY a.medical_group_id, a.plan_id
    ...
    JOIN int_aggregate_visits b
        ON a.medical_group_id=b.medical_group_id AND a.plan_id=b.plan_id 
),
